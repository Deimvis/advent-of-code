...
def find_invalids_sum(lb: int, ub: int):
    ...
    def go(cur_digits: list[int], i: int, cur_gt_lb: bool, cur_lt_ub: bool, is_zero: bool):
        ...
        if len(cur_digits) > 0:
            curmult = 2
            while len(cur_digits) * curmult <= n:
                cand = value(cur_digits * curmult)
                # print('cand', cand)
                if cand >= lb and cand <= ub and cand not in seen:
                    # print('found', cand)
                    ans += cand
                    seen.add(cand)
                curmult += 1
...
def brute_force(lb: int, ub: int):
    ...
    for x in range(lb, ub+1):
        x_digits = digits(x)
        n = len(x_digits)
        for partlen in range(1, n//2+1):
            # print('partlen=', partlen)
            if n%partlen == 0:
                ok = True
                must = x_digits[:partlen]
                for offset in range(1, n//partlen):
                    # print('offset=', offset, 'n = ', n, 'n//partlen+1 =', (n//partlen)+1)
                    part = x_digits[partlen*offset:partlen*offset+partlen]
                    # print(f'{part} vs {must}')
                    if part != must:
                        # print(f'{part} vs {must} - fail')
                        ok = False
                        break
